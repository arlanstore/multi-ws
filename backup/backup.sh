...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/arlanstore/izinvps/ipuk/ip | grep $MYIP | awk '{print $2}')
...
LLatest=`date`
Get_Data () {
git clone https://github.com/arlanstore/backup.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    DATE=$(date +'%d %B %Y')
    cd /root/user-backup
    git config --global user.email "arlanvpnstore@gmail.com" &> /dev/null
    git config --global user.name "arlanstore" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m backup &> /dev/null
    git branch -M ipuk &> /dev/null
    git remote add origin https://github.com/arlanstore/backup
    git push -f https://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@github.com/arlanstore/backup.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/arlanstore/backup/ipuk/$NameUser/$NameUser.zip"
sleep 1
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1│${NC}              • t.me/arlanstore •                $COLOR1│$NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo
